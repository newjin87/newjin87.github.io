...
class ReportGenerator:
    ...
    def create_company_report(self, company_name: str, ticker: str, analysis: dict, advice: str, news_items: list) -> str:
        """
        ...
        # 1. Header
        content = [
            f"# 📊 Daily Investment Analysis: {company_name} ({ticker})",
            f"**Date**: {today_str}",
            "---",
            ""
        ]
        ...
        content.append(f"**AI Sentiment Score**: {score}/100 ({sentiment_emoji})")
        content.append("")
        
        # 4. Keyword Analysis (User Request)
        content.append("## 🔑 Key Topics & News Summary")
        ...
        if topics:
            ...
            content.append("*No specific topics identified.*")
            content.append("")

        # 5. Bull/Bear
        content.append("## ⚖️ Bull vs Bear")
        content.append("**✅ Bullish Factors (Good News)**")
        for item in analysis.get("bullish_factors", []):
            content.append(f"- {item}")
        content.append("")
        
        content.append("**⚠️ Bearish Factors (Risk Factors)**")
        for item in analysis.get("bearish_factors", []):
            content.append(f"- {item}")
        content.append("")

        # 6. Investment Advice
        content.append("## 💡 Investment Advice")
        content.append(advice) # Advisor output is expected to be md formatted
        content.append("")
        
        # 7. Reference News
        content.append("## 🔗 Reference News (Source)")
        ...
        content.append("")
        content.append("---")
        content.append("*Disclaimer: This report is generated by AI (Gemini) and does not constitute financial advice.*")
